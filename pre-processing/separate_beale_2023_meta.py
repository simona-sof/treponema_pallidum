...
def separate_meta(df):
    ...
    new_df['authors'] = new_df['authors'].replace('This_Study', 'Beale_2023')
    new_df['sample_type'] = df['Sample_Type']
    new_df['reads_or_assemblies'] = df['Reads_or_assemblies']
    return new_df

def main():
    df = pd.read_excel("pre-processing/beale_2023.xlsx", header=0)
    ...
    SS14.to_csv('pre-processing/meta/meta_beale_2023_SS14.csv', index=False)
    Nichols.to_csv('pre-processing/meta/meta_beale_2023_Nichols.csv', index=False)
